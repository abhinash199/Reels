...
import HowItWorks from "@/components/organisms/VideoCall/HowItWorks";
...
const VideoCall: React.FC<VideoCallRequestProps> = () => {
  const router = useRouter();
  const [toggleClass, setToggleClass] = useState(false);
  const { id, profile, theme, videoCallIntro } = useArtist();
  ...
  return (
    <MainLayout title={"Video call "} activeCode={'video-call'}>
      <div id="ve-page" className={`header-footer-enabel ${toggleClass ? "call-request" : ""}`}>
        ...
        <div className="ve-container">
          {videoCallIntro?.url ? handlePlayer() : <p style={{color: theme?.secondary_color}}>Artist’s video explaining Video Call</p>}
        </div>
        ...
        >
          ...
          <div className="text-center">
            ...
            <h3 style={{color: theme?.secondary_color}}>ONE-TO-ONE VIDEO CALL WITH {profile?.first_name}</h3>
          </div>

          <button
            className="btn btn-primary d-block"
            onClick={() => {
              router.push(Routes.videoCallBooking(profile?.slug));
            }}
